...
//	static <T> T stringToObjectCollection(
	...
	/** I wonder... are the likelihoods generated by the masses in sync with the likelihoods normally combined? */
	...
	public DSRandVar combineWith(DSRandVar rv2) {
		...
		Set subsetOfSingletons = domain.getPossibleValues();
		Set<SubsetInfo> siSetNew = new HashSet<SubsetInfo>(subsetOfSingletons.size());
		double likelyTotal = 0;
		for (Object singleObj : subsetOfSingletons) {
		...
		for (SubsetInfo sinew : siSetNew) // normalize sinew
